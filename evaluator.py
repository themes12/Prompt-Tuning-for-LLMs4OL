...
if __name__=="__main__":
    ...
    if len(report['results']) == 0 or "bloom" in config.model or "flan" in config.model or "llama" or "llama3" in config.model:
        if config.model == "gpt3":
            ...
            for output in outputs:
                ...
                predictions.append(predict_list)
                labels.append(label_list)
        elif "bloom" in config.model or "flan" in config.model or "llama" in config.model or "llama3" in config.model:
            predictions, labels = [], []
            for output in outputs:
                label_list = output['label']
                predict = clean_text(output['pred'][0])
                predict_list = []
                for label in label_list:
                    # if label.lower() in predict:
                    if predict != "" and (label.lower() in predict or predict in label.lower()): # ???
                        predict_list.append(label)
                predictions.append(predict_list)
                labels.append(label_list)
        else:
