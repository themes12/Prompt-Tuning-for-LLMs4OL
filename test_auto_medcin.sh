...
datasets=("medcin")
#datasets=("wn18rr")
#templates=("template-2" "template-6" "template-7" "template-8")
...
for kb_name in "${datasets[@]}"; do
  ...
  for model_name in  "${models[@]}"; do
    ...
    for template in "${templates[@]}"; do
        ...
        #CUDA_VISIBLE_DEVICES=0 python3 test.py --kb_name=$kb_name --model_name=$model_name --template=$template --device=$device
